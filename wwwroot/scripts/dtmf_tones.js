...
$(".dialpad_button").on("mousedown touchstart", function(e){
	//e.preventDefault();

	var keyPressed = $(this).data('dpval'); // this gets the number/character that was pressed
	if ((isInt(keyPressed)) || (keyPressed === 'Z') || (keyPressed === '*') || (keyPressed === '#')) {
		var frequencyPair;
		if (keyPressed === 'Z') {
			frequencyPair = dtmfFrequencies[0]; 
		} else {
			frequencyPair = dtmfFrequencies[keyPressed]; // this looks up which frequency pair we need
		}
